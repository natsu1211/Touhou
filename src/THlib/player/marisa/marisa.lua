...
function marisa_player:shoot()
	self.nextshoot=4
	New(marisa_bullet,'marisa_bullet',self.x+6,self.y,24,90,1.25)
	New(marisa_bullet,'marisa_bullet',self.x-6,self.y,24,90,1.25)
	if self.fire==1 then
		for i=1,4 do if self.sp[i] and self.sp[i][3]>0.5 then
			--CreateMarisaLaser(self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],90,0.125)
			CreateMarisaLaser(self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],90,0.14)
		end end
	end
end
	...
	New(tasker,function()
		...
		for i=1,120 do
			...
			for i=1,3 do
				...
				for j=1,8 do
					New(player_bullet_hide,16,16,self.x,self.y,12,a+2.1*j,0.4,2*j)
					New(player_bullet_hide,16,16,self.x,self.y,12,a-2.1*j,0.4,2*j)
				end
			end
			task.Wait(2)
...
function marisa_player:render()
	...
	--laser light
	for i=1,4 do if self.sp[i] then
		Render('marisa_laser_light',self.supportx+self.sp[i][1],self.supporty+self.sp[i][2],self.ani*5,self.sp[i][3]*sz*self.fire*0.8)
	end end
